...
int DutVerilatorBase::Step(uint64_t ncycle, bool dump)
{
    ...
    if (dump) {
        for (int i = 0; i < ncycle; i++) {
            ...
            VerilatedVpi::callValueCbs(); 
#endif
            ((VSimTop *)(top))->contextp()->timeInc(1);
        }
    } else {
        assert(ncycle == 1);
        ((VSimTop *)(top))->eval_step();
    }
#if defined(VL_VPI)
    VerilatedVpi::callValueCbs(); 
#endif
...
int DutVerilatorBase::Finish()
{
    // Finish Verilator context
    if (this->top != nullptr) {
        VerilatedContext *contextp = ((VSimTop *)(this->top))->contextp();
#if defined(VL_COVERAGE)
        if (this->coverage_file_path.size() > 0)
            contextp->coveragep()->write(this->coverage_file_path.c_str());
        else
            contextp->coveragep()->write("VSimTop_coverage.dat");
#endif
        ((VSimTop *)(this->top))->final();
        delete (VSimTop *)(this->top);
        delete contextp;
        this->top = nullptr;
    }
    return 0;
...
{
#if defined(VL_TRACE)
    ((VSimTop *)(this->top))->contextp()->dumpfile(filename);
    ((VSimTop *)(this->top))->rootp->vlSymsp->_traceDumpOpen();
#else
...
#if defined(VL_TRACE)
    if(((VSimTop *)(this->top))->rootp->vlSymsp->__Vm_dumperp)return false;
    ((VSimTop *)(this->top))->rootp->vlSymsp->_traceDumpOpen();
#else
    std::cerr << "Verilator waveform is not enabled";
#endif
...
#if defined(VL_TRACE)
    if(!((VSimTop *)(this->top))->rootp->vlSymsp->__Vm_dumperp)return false;
    ((VSimTop *)(this->top))->rootp->vlSymsp->__Vm_dumperp->flush();
    ((VSimTop *)(this->top))->rootp->vlSymsp->_traceDumpClose();
#else
    std::cerr << "Verilator waveform is not enabled";
#endif
    return true;
...
std::string DutUnifiedBase::GetXSignalCFGPath()
{
    ...
    free(path);
    return  parent + "/SimTop_offset.yaml";
}

uint64_t DutUnifiedBase::GetXSignalCFGBasePtr()
{
#ifdef USE_VERILATOR
    if(0 == strcmp("MEM_DIRECT", "MEM_DIRECT")){
        return (uint64_t)((VSimTop *)(this->dut->top))->rootp;
    }
#endif
    return 0;
}
